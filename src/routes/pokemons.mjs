...
pokemonsRouter.post("/", (req, res) => {
    Pokemon.create(req.body)
        .then((createdPokemon) => {
            const message = `Le pokemon ${createdPokemon.name} a bien été crée !`;
...
pokemonsRouter.put("/:id", (req, res) => {
    const message = `La modification des pokemons est interdite !`;
    res.status(403).json({ message });
});

// Supprimer un pokemon.
pokemonsRouter.delete("/:id", (req, res) => {
    const message = `La suppression des pokemons est interdite !`;
    res.status(403).json({ message });
});
